...
- **The literal Ship-construction-scene visual (design doc Section 10.1/10.5-10.7) is superseded — see design doc Section 18, "The Voyage Map."** In-session testing showed a hand-coded flat-vector dock/ship scene reads as generic/amateur, not intentional — too much illustrative detail for what SVG-in-code does well. The replacement concept is a winding path of unlocked destinations (Duolingo/Candy Crush-style), one per Capítulo, reusing the same node-and-route pattern at a smaller scale for a Goal's Milestones too (design doc Section 18.2). The Area → Ship Part mapping (Section 10.2) is NOT retired — it's already shipped as `AreaIcon.tsx`'s icon glyphs. Still not part of the current Phase 1 MVP build order — use a simple placeholder (a toast/banner reading "Nivel up!" or "Chapter complete!") for now; the Voyage Map is a deliberate next phase, same as the Ship note it replaces.
